...
const AddTask = ({ refetch }) => {
  ...
  return (
    <form className="bg-[#FFFFFF] mx-auto relative h-[48px] w-[343px] lg:w-[570px] rounded-[8px] px-[14px] py-[10px] shadow-taskShadow flex items-center gap-[6px]">
      {toDoTextForEdit.value.length === 0 ? (
        <button
          onClick={(e) => addTask(e, user.id, value, refetch)}
        ...
        <button
          className="absolute left-[20px] w-[24px] h-[24px] flex items-center justify-center cursor-pointer"
          onClick={(e) => editTask(e, value, refetch)}
        >
          {<LiaEdit />}
        </button>
      )}
